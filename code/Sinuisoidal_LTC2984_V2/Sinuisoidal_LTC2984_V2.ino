...
#define USE_DIFFERENTIAL_TCS 1
  ...
  #define NUM_TCs 4
  const uint8_t LTC_TC_CHANNELS[NUM_TCs] = {4, 6, 8, 10};
#else
  #define NUM_TCs 12
...
bool wait_for_ltc_ready(uint32_t timeout_ms = 100)
{
  uint32_t t0 = millis();
...
// Kept for debugging, but no longer used in the timed sample path.
bool read_ltc_temperature(uint8_t channel, double &temperature_c, uint8_t &fault)
{
  transfer_byte(CHIP_SELECT, WRITE_TO_RAM, COMMAND_STATUS_REGISTER, 0x80 | channel);

  if (!wait_for_ltc_ready(100))
  {
    temperature_c = NAN;
    fault = 0x80;
    return false;
  ...
  temperature_c = convert_ltc_raw_to_temp(raw);
  return true;
}
...
void WriteToSD(void)
{
  ...
  if ((sdWriteCount % SD_FLUSH_INTERVAL) == 0)
  {
    logfile.flush();
...
void loop()
{
  ...
  {
    // Non-blocking change:
    // Do not call read_all_ltc_temperatures() here.
    // temp[] already contains the latest finished LTC conversions.
